...
```sh
python3 skill-eval-harness/skill_benchmark.py validate audit-skill/evals/shared-benchmark.json
```

Prepare paired run tasks:

```sh
python3 skill-eval-harness/skill_benchmark.py prepare audit-skill/evals/shared-benchmark.json --split tune --out /tmp/audit-skill-tasks.jsonl
```

Include ablation variants when running a focused regression check:

```sh
python3 skill-eval-harness/skill_benchmark.py prepare audit-skill/evals/shared-benchmark.json --split tune --include-ablations --out /tmp/audit-skill-ablation-tasks.jsonl
```
...
```sh
python3 skill-eval-harness/skill_benchmark.py benchmark audit-skill/evals/shared-benchmark.json --runs audit-skill/eval-runs/latest --out /tmp/audit-skill-benchmark.json
```
